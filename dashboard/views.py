         ...
         for l in list_of_csv:
            ...
            if len(l) == 3:  # Check if the row has exactly three columns
                product = Product.objects.create(
                    Bookname=l[0],       # Assuming the name is in the first column (index 0)
...
def products(request): #############################################################################################################
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                form.save()
                product_name = form.cleaned_data.get('Bookname')  # Update to book_name
    ...
    def insertion_sort(items):
        ...
        for i in range(1, len(items_list)):
            key = items_list[i]
            j = i - 1
            # Use book_name instead of name
            while j >= 0 and key.Bookname < items_list[j].Bookname:
    ...
    # Pass sorted items to the template
    context = {
        'items': sorted_items,
        'form': form,
    }
